...
'''
#banner for command promp work not needed now
def banner():
    print(r'Find interesting Subdomains and secrets hidden in page, folder, External Javascripts')
...
#using the google as a search engine
...
#using the ASK.com as search engine
...
#using the Baidu as search engine
...
# using the netcraft api as enumerator
...
#usinng the DNS dumper for enumeration
...
#using the CRT.sh as an enumerator
...
#passive dns as an enumerating engine
...
def main(domain, silent, verbose):
...
res= main(domain='acropolis.in',silent=False,verbose=False)
print(res)
